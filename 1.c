...
void initializeInvaders(Invader invaders[5][11]);
void updateInvaders(Invader invaders[5][11]);

void drawInvaders(SDL_Renderer *ren,SDL_Texture *tex,Invader invaders[5][11]);

int main()
{
  Invader invaders[5][11];
  ...
  SDL_Window *win = 0;
  win = SDL_CreateWindow("Invaders", 100, 100, 800, 600, SDL_WINDOW_SHOWN);
  if (win == 0)
  {
    printf("%s\n",SDL_GetError());
    return 1;
...
void updateInvaders(Invader invaders[5][11])
{
  ...
  int yinc=0;
  if(invaders[0][11].pos.x>=(800 -30)-(11*(30 +10)))
  {
    DIRECTION=BWD;
    yinc=10;

  }
  else if(invaders[0][0].pos.x<=30)
  {
    DIRECTION=FWD;
    yinc=10;
